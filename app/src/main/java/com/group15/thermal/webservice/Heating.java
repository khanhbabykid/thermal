...
public class Heating {
	...
	public static String BASE_ADDRESS = "http://thermalstatws.herokuapp.com/015";
	...
	public static WeekProgram getWeekProgram() throws ConnectException,
		...
		try {
			HttpURLConnection connect = getHttpConnection(
					Heating.WEEK_PROGRAM_ADDRESS, "GET");
			...
			System.out.println("All switches done!");
			return program;
	...
	private static HttpURLConnection getHttpConnection(String link, String type)
		...
		connect.setReadTimeout(Heating.TIME_OUT);
		connect.setConnectTimeout(Heating.TIME_OUT);
	...
	public static String get(String attribute_name) throws ConnectException,
		...
		// Week you need to call getWeekProgram().
		String link = "";
		boolean match = false;
		...
		for (i = 0; i < valid_names.length; i++) {
			if (attribute_name.equalsIgnoreCase(valid_names[i])) {
				match = true;
				link = Heating.BASE_ADDRESS + "/" + valid_names[i];
				break;
			}
		}
	...
	public static void put(String attribute_name, String value)
		...
		// Perform Dimension checks. And check if we get a http response status
		// code of "OK".
	...
	public static boolean getVacationMode() {
		try {
			String state = Heating.get("weekProgramState");
			if (state.equals("on")) {
				return false;
			} else {
				return true;
	...
	public static void setWeekProgram(WeekProgram wpg) {
		...
		try {
			...
			//System.out.println("Link: " + Heating.WEEK_PROGRAM_ADDRESS);
			HttpURLConnection connect = getHttpConnection(
					Heating.WEEK_PROGRAM_ADDRESS, "PUT");
	...
	public static boolean startservice() {
		try {
			HttpURLConnection connect = getHttpConnection(BASE_ADDRESS,"PUT");
			int code = connect.getResponseCode();
			switch(code) {
			case 200:
				System.out.println(code +" ID exists");
				break;
			case 201:
